    ...
    <form onSubmit={sendPrompt} className={`w-full ${selectedChat?.messages.length> 0 ? 'max-w-3xl' : 'max-w-2xl'} bg-[#2a2a2e] p-4 mt-4 rounded-3xl transition-all border border-white/5`}>
      ...
      <div className='flex items-center justify-between text-sm'>
        <div className='flex items-center gap-2'>
          <p className='flex items-center gap-2  text-[12px] border border-gray-300/40 font-semibold  px-2.5 py-1 rounded-full cursor-not-allowed hover:bg-gray-300/10 transition' title='Think before responding to solve reasoning problems'>
            <Image src={assets.deepthink_icon} alt='' width={15} height={15} /> Deepthink
          </p>
          <p className='flex items-center gap-2 text-[12px] border border-gray-300/40  px-2 py-1 rounded-full cursor-not-allowed hover:bg-gray-300/10 transition' title='Search the web when necessary'>
            <Image src={assets.search_icon} alt='' height={15} width={15} /> Search
          </p>
        </div>

        <div className='flex items-center gap-5 '>
          <div className='cursor-pointer px-3 py-2.5 hover:bg-white/10 rounded-full '>
          <Image src={assets.pin_icon} alt='' height={10} width={10} />
          </div>
          <button className={` ${prompt ? 'bg-primary cursor-pointer' : "bg-[#71717a] cursor-not-allowed "} rounded-full p-2   `}>
            <Image className=' aspect-square' src={prompt ? assets.arrow_icon : assets.arrow_icon_dull} alt='' height={17} width={17} />
          </button>
        </div>
      </div>
